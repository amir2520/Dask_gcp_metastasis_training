...
build-prod-worker:
	docker build \
		-f docker/Dockerfile.dask-worker \
		--build-arg USER_ID=$(USER_ID) \
		--build-arg USER_NAME=$(USER_NAME) \
		-t metas-dask-worker:latest .
	docker tag metas-dask-worker:latest $(DOCKER_WORKER_IMAGE)

push-prod-worker: build-prod-worker
	gcloud auth print-access-token | docker login -u oauth2accesstoken --password-stdin https://us-east1-docker.pkg.dev
	docker push $(DOCKER_WORKER_IMAGE)


run-prod: push-prod-worker up-prod
	$(PROD_DOCKER_COMPOSE_EXEC) python metastatic/run_training.py -m +experiment=sweep_no_smote

run-local: up
	$(DOCKER_COMPOSE_EXEC) python metastatic/run_training.py
